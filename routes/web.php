...
if (app()->environment('local')) {
    ...
    Route::get('/test-404', fn () => abort(404))->name('test.404');
    Route::get('/test-403', fn () => abort(403))->name('test.403');
    Route::get('/test-500', fn () => response()->view('errors.500', [], 500))->name('test.500');
    Route::get('/test-real-500', fn () => throw new \Exception('Simulasi error 500'))->name('test.real.500');
    Route::get('/test-429', fn () => abort(429))->name('test.429');
}
...
Route::middleware('auth')->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::resource('/customers', CustomerController::class)->names('customers');
    Route::resource('/categories', CategoryController::class)->names('categories');
    Route::resource('/price-categories', PriceCategoryController::class)->except('show')->names('price-categories');
    Route::patch('/branches/{branch}/status', [BranchController::class, 'updateStatus'])->name('branches.update-status');
    Route::resource('/branches', BranchController::class)->names('branches');
    ...
    Route::middleware('role:manager')->group(function () {
        Route::resource('/employees', EmployeeController::class)->names('employees');
        Route::resource('/permissions', PermissionsController::class)->except(['show'])->names('permissions');
        Route::post('/permissions/{id}/regenerate-token', [PermissionsController::class, 'regenerateToken'])->name('permissions.regenerate-token');
        Route::patch('/permissions/{id}/status', [PermissionsController::class, 'updateStatus'])->name('permissions.update-status');
    });
